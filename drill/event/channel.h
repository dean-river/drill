...
class Channel {
 public:
  ...
  typedef std::function<void(drill::common::Time)> ReadEventCallback;
  Channel();
  Channel(EventLoop* loop, int sockfd);
  ~Channel();
  void setUp(EventLoop* loop, int sockfd)
  {_loop = loop;_fd = sockfd;}
  void handleEvent(drill::common::Time receiveTime);
 ...
 private:
  void update();
  void handleEventWithGuard(drill::common::Time receiveTime);
  ...
  int                   _fd;
  int                   _events;
  int                   _revents; // it's the received event types of epoll
