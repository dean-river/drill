...
struct timespec fromNow(drill::common::Time when)
{
	 drill::common::Time diff = when -  drill::common::Time::now();
  ...
  ts.tv_sec = static_cast<time_t>(
      microseconds /  drill::common::Time::kMicroSecondsPerSecond);
  ts.tv_nsec = static_cast<long>(
      (microseconds %  drill::common::Time::kMicroSecondsPerSecond) * 1000);
  return ts;
}

void readTimerfd(int timerfd, drill::common::Time now)
{
  (void)now;
  uint64_t howmany;
...
void resetTimerfd(int timerfd, drill::common::Time expiration)
{
...
Timer::Timer(EventLoop* loop)
  : _loop(loop),
    _timerfd(drill::detail::createTimerfd()),
...
void Timer::addTimerInLoop(TimerTask* task)
{
  ...
  if (earliestChanged)
  {
    drill::detail::resetTimerfd(_timerfd, task->expiration());
  }
}
...
{
  _loop->assertInLoopThread();
   drill::common::Time now( drill::common::Time::now());
  drill::detail::readTimerfd(_timerfd, now);
...
std::vector<Timer::Entry> Timer::getExpired(drill::common::Time now)
{
  assert(_tasks.size() == _activeTasks.size());
...
void Timer::reset(const std::vector<Entry>& expired,  drill::common::Time now)
{
   drill::common::Time nextExpire;
  ...
  if (nextExpire.valid())
  {
    drill::detail::resetTimerfd(_timerfd, nextExpire);
  }
}
...
bool Timer::insert(TimerTask* task)
{
  ...
  drill::common::Time when = task->expiration();
