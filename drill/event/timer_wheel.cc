...
{
	std::shared_ptr<TimerTask> wheelPtr(new TimerTask(factor, drill::common::Time::now() + interval * 1000000,
											repeat?interval:0));
	...
	if(_loop->isInLoopThread()) {
		...
		detail::WheelFunc func= std::bind(&TimerWheel::addTimerInLoop,this,std::placeholders::_1);
...
void TimerWheel::onTimer()
{
	...
	for(size_t i =0;i <mylist.size(); i++) {
		...
		if(ptr) {
			ptr->run();
			if(ptr->repeat()) {
				ptr->restart(drill::common::Time::now());
				addTimerInLoop(ptr);
