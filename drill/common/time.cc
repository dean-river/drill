...
Time Time:: dateString(const string & datestr, const char* spliter)
{
	...
	string_split(datestr, spliter, dateint);
	if(dateint.size() < 6) {
		return Time(0);
	}
	ptm.tm_year = string_to_int(dateint[0])- 1900;
    ptm.tm_mon = string_to_int(dateint[1]) - 1;
    ptm.tm_mday = string_to_int(dateint[2]);
    ptm.tm_hour = string_to_int(dateint[3]);
    ptm.tm_min = string_to_int(dateint[4]);
    ptm.tm_sec = string_to_int(dateint[5]);
	...
	if(dateint.size() == 7) {	
		...
		int64_t tail = string_to_int(dateint[6]);
		usec += tail;
	}
	return Time(usec);		
}
