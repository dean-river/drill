...
namespace detail {
	...
	drill::common::Time g_startTime = drill::common::Time::now();
}
	...
	drill::common::Time ProcessInfo::startTime()
	{
	...
	{
	  string result;
	  drill::common::ReadSmallFile status("/proc/self/status");
	...
	{
	  string result;
	  drill::common::ReadSmallFile status("/proc/self/stat");
	  status.read(&result, 65535);
	  return result;
	}
