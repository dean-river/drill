  ...
  if (space != end && request.setMethod(start, space))
  {
    ...
    if (space != end)
    {
      ...
      if (succeed)
      {
        if (*(end-1) == '1')
        {
          request.setVersion(kHttp11);
        }
        else if (*(end-1) == '0')
        {
          request.setVersion(kHttp10);
        }
        else
        {
          succeed = false;
    ...
    if (context->expectRequestLine()) {
      ...
      if (crlf) {
    	...
    	if(cachelen == 0) {
			...
			int64_t len = string_to_int(lenstr.c_str(),lenstr.length());
			if(len == 0 ) {
				context->receiveBody();
				hasMore = false;
				continue;
...
void defaultHttpCallback(const HttpRequest&, HttpResponse* resp)
{
  ...
  resp->setStatusMessage(getStatusString(not_found));
...
req)
{
  ...
  bool close = connection == "close" ||
    (req.getVersion() == kHttp10 && connection != "Keep-Alive");
